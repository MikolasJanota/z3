...
void bv_gauss_elim::add_term(const numeral& coef, app_ref& v, row& r) {
    if (v.get() == NULL) {
        r.coef = r.coef + coef;
    }
    else {
        coef_map::obj_map_entry * entry = r.coefs->insert_if_not_there2(v.get(), numeral::zero());
        entry->get_data().m_value = entry->get_data().m_value + coef;
    }
}
...
void bv_gauss_elim::add_row(expr * e) {
    ...
    if (min_rank > 0) {
        ...
        if (get_rank(r.coef) < min_rank) m_is_consistent = false;
